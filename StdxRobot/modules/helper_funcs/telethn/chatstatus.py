...
from StdxRobot import DRAGONS
from Stdxfrom telethon.tl.types import ChannelParticipantsAdmins

from StdxRobot import DRAGONS
from StdxRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def can_delete_messages(message):
    ...
    if message.is_private:
        ...
        return FalsehRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def stdx_is_admin(chat_id: int):
    status = False
    mukesh = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
